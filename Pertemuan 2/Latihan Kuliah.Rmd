...
data.ts<-ts(dtmp$price)
data.ts

#Membuat plot time series
ts.plot(data.ts, xlab="Time Period ", ylab="price", main= "Time Series Plot of price")
points(data.ts)
```
...
```{r}
dt.gab <- cbind(aktual = c(data.ts,rep(NA,3)), 
...
#Plot time series
ts.plot(dt.gab[,1], xlab="Time Period ", ylab="price", 
        main= "DMA N=4 Data price", ylim=c(2,10))
points(dt.gab[,1])
points(dt.gab[,3])
points(dt.gab[,6])
lines(dt.gab[,3],col="navy",lwd=2)
lines(dt.gab[,6],col="red",lwd=2)
legend("topleft",c("data aktual","data pemulusan","data peramalan"), 
       lty=8, col=c("black","navy","red"), cex=0.8)
```
...
plot(training.ts, col="navy",main="Plot data training")
points(training.ts)
```
